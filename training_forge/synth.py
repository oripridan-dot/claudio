...
class DDSPSynth(nn.Module):
    ...
    def forward(self, f0, amplitudes, harmonics, noise, reverb_mix=None, f0_residual=None, voiced_mask=None):
        ...
        if voiced_mask is not None:
            ...
            v_up = F.interpolate(voiced_mask.transpose(1, 2), size=audio_len, mode='linear', align_corners=False).transpose(1, 2)
        ...
        audio_len = time_frames * self.hop_length
        mix_up = F.interpolate(reverb_mix.transpose(1, 2), size=audio_len, mode='linear', align_corners=False).transpose(1, 2)
