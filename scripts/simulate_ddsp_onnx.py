...
from benchmark_resynthesis import measure_spectral_preservation, measure_transient_error, measure_snr_approx
...
def run_stress_test():
    ...
    for idx, path in enumerate(sorted(audio_files)):
        ...
        for i in range(n_blocks):
            ...
            if frames:
                ...
                if len(mfcc_arr) < 13: mfcc_arr = mfcc_arr + [0]*(13-len(mfcc_arr))
                z_in = np.array([[mfcc_arr[:13]]], dtype=np.float32)

                # Measure purely the ML Inference
                t0_inf = time.perf_counter()
                
                outputs = session.run(['harmonics', 'noise'], {
                    'f0': f0_in,
                    'loudness': loud_in,
                    'z': z_in
                })
        ...
        print(f"✅ {name:<18} | Inf(Avg): {avg_inf_ms:.3f}ms | Inf(P99): {p99_inf_ms:.3f}ms | SpecCorr: {spec_pres:.3f} | RTF: {rtf:.1f}x")
    ...
    # Save artifact
    report_path = root_dir / ".gemini" / "antigravity" / "brain" / "48506ba8-c903-480f-8f8a-0866d0e38be3" / "ddsp_stress_report.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    
    with open(report_path, "w") as f:
        f.write("# Neural Vocoder (DDSP ONNX) Stress Simulation Report\n\n")
        f.write("Tested on Pure Intent Architecture (v3.0) via `onnxruntime` CPU provider mimicking WebNN constraints.\n\n")
        f.write("| Instrument | Avg Latency | P99 Latency | Spectral Corr | SNR (approx) | RTF |\n")
        f.write("|------------|-------------|-------------|---------------|--------------|-----|\n")
        for r in results:
            f.write(f"| {r['name']} | {r['avg_inf']:.3f}ms | {r['p99_inf']:.3f}ms | {r['spec_pres']:.3f} | {r['snr']:.1f} dB | {r['rtf']:.1f}x |\n")
