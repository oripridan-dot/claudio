...
def load_and_align(ref_path: str, synth_path: str, target_sr: int = 48000):
    ...
    if synth_wav.shape[0] > 1: synth_wav = torch.mean(synth_wav, dim=0, keepdim=True)
...
def calculate_mcd(ref_wav: torch.Tensor, synth_wav: torch.Tensor, sr: int) -> float:
    ...
    mfcc_transform = T.MFCC(
        sample_rate=sr,
        n_mfcc=13,
        melkwargs={"n_fft": 2048, "hop_length": 512, "n_mels": 80}
    )
...
def plot_missing_frequencies(ref_wav: torch.Tensor, synth_wav: torch.Tensor, sr: int, save_path="fidelity_delta.png"):
    ...
    plt.imshow(delta_db.numpy(), aspect='auto', origin='lower', cmap='coolwarm', 
               vmin=-20, vmax=20, extent=[0, ref_wav.shape[1]/sr, 0, sr/2000])
...
if __name__ == "__main__":
    ...
    if mcd_score < 4.0: print("    -> Excellent timbre match.")
    elif mcd_score < 8.0: print("    -> Good, but slight 'plastic' coloration.")
    else: print("    -> Poor timbre tracking. Investigate DDSP harmonic mappings.")
    ...
    if lsd_score < 1.0: print("    -> SOTA texture rendering. Breaths and friction are perfect.")
    elif lsd_score < 2.5: print("    -> Decent, but missing some high-end 'air'.")
    else: print("    -> Missing complex textures. Synthesizer sounds robotic/sterile.")
