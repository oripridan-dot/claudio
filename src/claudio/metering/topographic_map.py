...
class TopographicFreqMap:
    ...
    def compute(
        ...
        for i in range(len(ids)):
            for j in range(i + 1, len(ids)):
                ...
                if np.any(collide):
                    ...
                    severity = "critical" if col_db > 18 else "medium" if col_db > 10 else "low"
        ...
        mud_score = min(1.0, len(collision_zones) * 0.15 + sum(z.collision_db / 40.0 for z in collision_zones))
