...
class GestureClassifier:
    ...
    def _check_dynamic_vertical(self, frame: LandmarkFrame) -> GestureEvent | None:
        ...
        if abs(vy_r) > self.VEL_THRESHOLD and abs(vy_l) > self.VEL_THRESHOLD:
            ...
            if same_dir:
                ...
                gt = GestureType.RAISE_BOTH_HANDS if vy_r < 0 else GestureType.LOWER_BOTH_HANDS
                mag = (abs(vy_r) + abs(vy_l)) / 2
                return GestureEvent(
                    gesture=gt,
                    confidence=min(1.0, mag / (self.VEL_THRESHOLD * 3)),
                    timestamp=frame.timestamp,
                    hand="both",
                    magnitude=mag,
                )
    ...
    # ── Static gesture heuristics (extracted to gesture_heuristics.py) ────────

    @staticmethod
    def _is_open_palm(lm: np.ndarray) -> bool:
        from .gesture_heuristics import is_open_palm

        return is_open_palm(lm)

    @staticmethod
    def _is_fist(lm: np.ndarray) -> bool:
        from .gesture_heuristics import is_fist
